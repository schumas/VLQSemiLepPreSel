...
static const float DPT_2D_CUT_PRESEL = 25.0;

static const float prim_lep_min_ele_pt = 55.;
static const float prim_lep_min_mu_pt = 55.;
...
static const vector<shared_ptr<SelectionItem>> SEL_ITEMS_PRESEL {
  //  shared_ptr<SelectionItem>(new SelDatF("primary_lepton_pt", ";primary lepton p_{T}; events / 20 GeV",    40, 0, 800     , 20    )),
    shared_ptr<SelectionItem>(new SelDatF("leading_jet_pt",    ";leading jet p_{T}; events / 20 GeV",       40, 0, 800   , 185        )),
      //    shared_ptr<SelectionItem>(new SelDatD("ST",                ";ST; events / 50 GeV",                      50, 0, 2500          )),
      //    shared_ptr<SelectionItem>(new SelDatI("n_hcands",          ";N ak8 jet with mass > 40 GeV; events",     6, -.5, 5.5           )),
